...
<template>
  <section class="flex-preview-shell" aria-label="ตัวอย่าง LINE Flex Message">
    ...
    <article class="flex-preview-card">
      <header class="flex-preview-header">NEXTSTEP DASHBOARD</header>
      <div class="flex-preview-body">
        ...
        <section v-for="report in preview.reports" :key="report.key" class="flex-preview-report">
          <h4>{{ report.label }}</h4>
          <div v-for="metric in report.metrics" :key="metric.label" class="flex-preview-metric">
      ...
      <footer class="flex-preview-footer">
        <span role="button" aria-disabled="true">เปิดรายงาน</span>
      </footer>
    </article>
...
.flex-preview-report h4 { margin: 0 0 0.55rem; color: #0f766e; font-size: 0.95rem; }
