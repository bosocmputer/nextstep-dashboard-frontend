...
<template>
  <div class="page-header"><div><h1 class="page-title">Report Runs</h1><p class="page-subtitle">สถานะ execution และ data-quality metadata โดยไม่แสดง credentials</p></div><Button label="รีเฟรช" icon="pi pi-refresh" outlined :loading="loading" @click="load()" /></div>
  <form class="surface-card rounded-xl p-4 grid grid-cols-1 md:grid-cols-[1fr_14rem_auto] gap-3 mb-4" @submit.prevent="load()"><InputText v-model="tenantId" aria-label="กรองด้วย Tenant UUID" placeholder="Tenant UUID (ไม่บังคับ)" fluid /><Select v-model="status" aria-label="กรองสถานะ Run" :options="statuses" show-clear placeholder="ทุกสถานะ" fluid /><Button type="submit" label="กรอง" icon="pi pi-filter" outlined /></form>
  <Message v-if="error" severity="error" :closable="false" class="mb-4">{{ error }}</Message>
  <div class="surface-card rounded-xl overflow-hidden"><DataTable :value="rows" :loading="loading" data-key="id" striped-rows scrollable><Column field="reportKey" header="รายงาน" frozen><template #body="{ data }"><div class="font-medium">{{ reportDefinitionByKey.get(data.reportKey)?.label ?? data.reportKey }}</div><code class="text-xs text-muted-color">{{ data.id }}</code></template></Column><Column field="status" header="สถานะ"><template #body="{ data }"><Tag :severity="severity(data.status)" :value="data.status" /></template></Column><Column header="Period"><template #body="{ data }">{{ data.dateFrom || '—' }}<span v-if="data.dateTo && data.dateTo !== data.dateFrom"> → {{ data.dateTo }}</span></template></Column><Column field="rowCount" header="Rows"><template #body="{ data }"><span class="metric-value">{{ data.rowCount.toLocaleString('th-TH') }}</span></template></Column><Column field="safeErrorCode" header="Safe error"><template #body="{ data }"><code v-if="data.safeErrorCode" class="text-red-600">{{ data.safeErrorCode }}</code><span v-else>—</span></template></Column><Column field="queuedAt" header="Queued"><template #body="{ data }">{{ formatDateTime(data.queuedAt) }}</template></Column><Column field="finishedAt" header="Finished"><template #body="{ data }">{{ formatDateTime(data.finishedAt) }}</template></Column><template #empty><div class="py-8 text-center text-muted-color">ยังไม่มี Report Run</div></template></DataTable><div v-if="hasMore" class="p-4 border-t border-surface text-center"><Button label="โหลดเพิ่มเติม" outlined :loading="loading" @click="load(false)" /></div></div>
</template>
