...
<template>
  ...
  <template v-else-if="tenant && catalog">
    ...
    <form class="flex flex-col gap-5" @submit.prevent="save">
      ...
      <div class="card"><div class="flex flex-wrap justify-end gap-2"><Button type="button" label="ยกเลิก" text class="touch-action" @click="back" /><Button type="button" label="ดูตัวอย่าง LINE Card" icon="pi pi-eye" outlined class="touch-action" :disabled="!form.reportKeys.length || form.reportKeys.length > maxReports" :loading="previewing" @click="createPreview" /><Button type="submit" label="บันทึกตารางส่ง" icon="pi pi-save" class="touch-action" :disabled="!valid || readOnly" :loading="saving" /></div></div>
    </form>
  </template>
</template>
