...
<template>
  ...
  <template v-if="run?.status === 'SUCCEEDED'">
    ...
    <div class="surface-card rounded-2xl overflow-hidden"><div class="p-4 border-b border-surface flex flex-wrap items-center justify-between gap-2"><div><h2 class="text-lg font-semibold m-0">รายละเอียด</h2><p class="text-xs text-muted-color mt-1 mb-0">สร้าง {{ formatDateTime(run.finishedAt) }} · แถวชั่วคราวเก็บ 24 ชั่วโมง</p></div><Tag v-if="run.isTruncated" severity="warn" value="ผลลัพธ์ถูกตัดตาม row limit" /></div><div class="hidden md:block"><DataTable :value="rows" :loading="loadingRows" scrollable striped-rows><Column v-for="column in columns" :key="column" :field="column" :header="metricLabel(column)"><template #body="{ data }"><span class="safe-wrap metric-value">{{ formatMetric(data[column]) }}</span></template></Column><template #empty><div class="py-8 text-center text-muted-color">รายงานนี้ไม่มีข้อมูลในช่วงที่เลือก</div></template></DataTable></div><div class="md:hidden p-3 grid gap-3" aria-label="รายละเอียดรายงานแบบมือถือ"><article v-for="(row, rowIndex) in rows" :key="rowIndex" class="rounded-xl border border-surface p-4 grid gap-2"><div v-for="column in columns" :key="column" class="flex items-start justify-between gap-4"><span class="text-xs text-muted-color safe-wrap">{{ metricLabel(column) }}</span><strong class="text-sm text-right metric-value safe-wrap">{{ formatMetric(row[column]) }}</strong></div></article><div v-if="!loadingRows && !rows.length" class="py-8 text-center text-muted-color">รายงานนี้ไม่มีข้อมูลในช่วงที่เลือก</div></div><div v-if="hasMore" class="p-4 text-center border-t border-surface"><Button label="โหลดอีก 100 แถว" icon="pi pi-angle-down" outlined :loading="loadingRows" @click="loadRows(false)" /></div></div>
  </template>
</template>
